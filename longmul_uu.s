...
@ Long multiplication (32-bit inputs, 64-bit output), unsigned
@ Cost: 20 cycles
@ ========================================================================
	.align	1
	.global	longmul_uu
...
longmul_uu:
	...
	@ As in longmul_ss(), we split x1*y0 into low and high halves.
	@ All values are unsigned here:
	@      x0*y1   <= (2^16 - 1)^2
	@    lo(x1*y0) <= 2^16 - 2
	@ hence:
	@    x0*y1 + lo(x1*y0) <= 2^32 - 2^16 - 1 < 2^32 - 1
